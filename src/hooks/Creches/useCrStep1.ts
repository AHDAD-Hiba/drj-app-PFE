...
export function useCrDemandesLicences(rapportId: string | null, options?: { enabled?: boolean }) {
...
export function useCrTraitementLicences(rapportId: string | null, options?: { enabled?: boolean }) {
...
export function useDirCrechesPrivees(directionId?: string) {
  ...
  const add = async (entry: Omit<DirCrechePriveeEntry, "id">) => {
    ...
    if (data) {
      setItems((prev) =>
        prev.map((item) => (item.local_id === entry.local_id ? { ...item, id: data.id } : item)),
      );
    }
  };
  ...
  const update = async (local_id: string, updates: Partial<DirCrechePriveeEntry>) => {
    setItems((prev) =>
      prev.map((item) => (item.local_id === local_id ? { ...item, ...updates } : item)),
    );
