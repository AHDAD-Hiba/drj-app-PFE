...
export interface EnfanceCrechesSection6Data {
  ...
  mouvements: {
    ...
    details: { secteur: string; type: string; nombre: number; raisons: string }[];
  };
  cadres: {
    totalCadres: number;
...
export interface EnfanceCrechesDashboardData {
  status: {
  ...
  section3: EnfanceCrechesSection3Data;
  evolution: EnfanceCrechesEvolutionData;
  benchmark: PrefDomainBenchmarkRow[];
  detailed: EnfanceCrechesSection6Data;
}
...
const buildBenchmark = (kpis: EnfanceCrechesKpisRaw): PrefDomainBenchmarkRow[] => [
  { kpi: "Enfants pris en charge", monScore: kpis.enfantsPrisesEnCharge, moyenneReg: 0, isPercentage: false },
  { kpi: "Demandes de licences traitées", monScore: kpis.demandesLicencesTraitees, moyenneReg: 0, isPercentage: false },
  { kpi: "Délai moyen de traitement (jours)", monScore: kpis.delaiMoyenTraitementJours, moyenneReg: 0, isPercentage: true },
  { kpi: "Cadres assermentés", monScore: kpis.cadresAssermentes, moyenneReg: 0, isPercentage: false },
  { kpi: "Crèches labellisées qualité (obtenues)", monScore: kpis.crechesLabelliseesQualite, moyenneReg: 0, isPercentage: false },
  { kpi: "Fermetures de crèches signalées", monScore: kpis.fermeturesCrechesSignalees, moyenneReg: 0, isPercentage: false },
];
...
const buildDetailed = (
  ...
  const demandesParType = Array.from(
    ...
    .map(([id, value]) => ({ id, name: typeById.get(id)?.libelle_fr || typeById.get(id)?.libelle_ar || id, value }))
    .sort((a, b) => b.value - a.value);
  ...
  const demandesParStatut = Array.from(
    ...
    .map(([id, value]) => ({ id, name: statutById.get(id)?.libelle_fr || statutById.get(id)?.libelle_ar || id, value }))
    .sort((a, b) => b.value - a.value);
  ...
  const detailsMouvements = mouvements.map((row) => ({
    secteur: row.secteur || "non_renseigne",
...
export const loadEnfanceCrechesDashboard = async (
  ...
  const [
    ...
    loadStatistics(rapportIds),
    loadTraitementLicences(rapportIds),
    loadDemandesLicences(rapportIds),
    loadCadresAssermentes(rapportIds),
    loadLabelQualite(rapportIds),
    loadMouvementsFermetures(rapportIds),
