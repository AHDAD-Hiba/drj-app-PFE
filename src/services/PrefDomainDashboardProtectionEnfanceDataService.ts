...
export type ProtectionEnfanceDashboardData = DashboardData<
...
const buildBenchmark = (
  ...
  regionalAverages: Partial<Record<keyof ProtectionEnfanceKpisRaw, number>> = {},
): PrefDomainBenchmarkRow[] => {
  const regional = (key: keyof ProtectionEnfanceKpisRaw, fallback = 0) =>
...
export const loadProtectionEnfanceDashboard = async (
  ...
  const [
    ...
    dons,
    amenagementEquipement,
    rapportsExceptionnelsAll,
    rapportsJudiciaires,
    typesIncident,
    domainesActivite,
  ] = await Promise.all([
    loadStatus(latestRapport.id, domaineId),
    loadPeStatistiquesDemographiques(fetchIds),
    loadPeEducation(fetchIds),
    loadPeFormationBeneficiaires(rapportIds),
    loadPeAteliersCrees(rapportIds),
    loadPeLiberteSurveillee(fetchIds),
    ...
    loadPeRapportsExceptionnels(fetchIds),
  ...
  const education = filterByRapportIds(educationAll, localRapportIdSet);
  const liberteSurveillee = filterByRapportIds(liberteSurveilleeAll, localRapportIdSet);
  const rapportsExceptionnels = filterByRapportIds(rapportsExceptionnelsAll, localRapportIdSet);
  const regionalStats = statsAll;
  const regionalEducation = educationAll;
  const regionalLiberteSurveillee = liberteSurveilleeAll;
  const regionalRapportsExceptionnels = rapportsExceptionnelsAll;
    ...
    regionalDirectionIds
      .map((regionalDirectionId) => {
        ...
        return buildKpis(
          ...
          regionalEducation.filter((row) => row.rapport_id && ids.has(row.rapport_id)),
          regionalLiberteSurveillee.filter((row) => row.rapport_id && ids.has(row.rapport_id)),
          regionalRapportsExceptionnels.filter((row) => row.rapport_id && ids.has(row.rapport_id)),
        );
      })
      .filter((value): value is ProtectionEnfanceKpisRaw => value !== null),
  ) as Partial<Record<keyof ProtectionEnfanceKpisRaw, number>>;
