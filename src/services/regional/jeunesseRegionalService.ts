...
export interface JeunesseRegionalEvolution {
  ...
  etablissements: { trimestre: string; fonctionnels: number | null; travaux: number | null; fermes: number | null }[];
}
...
const relativeScore = (directionValue: number, regionalValue: number) =>
  regionalValue > 0
    ? (directionValue / regionalValue) * 100
    : 0;
...
const calculateJeunesseScore = (
  ...
  const score = (Object.keys(JEUNESSE_SCORE_WEIGHTS) as Array<keyof JeunesseScoreKpis>)
    .reduce((total, kpi) => total + relativeScore(directionKpis[kpi], regionalKpis[kpi]) * (JEUNESSE_SCORE_WEIGHTS[kpi] / 100), 0);
...
const loadJeunesseDataset = async (rapportIds: string[], year: number): Promise<JeunesseDataset> => {
  ...
  if (rapportIds.length === 0) {
    return { activites: [], participants: [], formations: [], formationStats: [], festivals: [], festivalStats: [], insertions: [], insertionStats: [], partenariats: [], projets: [], demographie: demographieResult.data ?? [], etablissements: etablissementsResult.data ?? [] };
  }
  const [activitesResult, participantsResult, formationsResult, festivalsResult, insertionsResult, partenariatsResult, projetsResult] = await Promise.all([
  ...
  const [formationStatsResult, festivalStatsResult, insertionStatsResult] = await Promise.all([
    formations.length ? supabase.from("statistiques_formation").select("*").in("formation_id", formations.map((row) => row.id)) : Promise.resolve({ data: [] as FormationStatsRow[] }),
    festivals.length ? supabase.from("statistiques_festivals").select("*").in("festival_id", festivals.map((row) => row.id)) : Promise.resolve({ data: [] as FestivalStatsRow[] }),
    insertions.length ? supabase.from("stats_insertion").select("*").in("activite_id", insertions.map((row) => row.id)) : Promise.resolve({ data: [] as InsertionStatsRow[] }),
  ]);
  return {
    activites: activitesResult.data ?? [], participants: participantsResult.data ?? [], formations,
    formationStats: formationStatsResult.data ?? [], festivals, festivalStats: festivalStatsResult.data ?? [],
    insertions, insertionStats: insertionStatsResult.data ?? [], partenariats: partenariatsResult.data ?? [], projets: projetsResult.data ?? [],
    demographie: demographieResult.data ?? [], etablissements: etablissementsResult.data ?? [],
  };
};

const aggregateJeunesse = (rapports: JeunesseRapportRow[], dataset: JeunesseDataset, directionId?: string): JeunesseScoreKpis & JeunesseRegionalKpis & { section3: JeunesseRegionalSection3; activityEvolution: JeunesseRegionalEvolution["activites"]; establishmentEvolution: JeunesseRegionalEvolution["etablissements"] } => {
  const scopedReports = directionId ? rapports.filter((rapport) => rapport.direction_id === directionId) : rapports;
  const reportIds = new Set(scopedReports.map((rapport) => rapport.id));
  const rowsForReports = <T extends { rapport_id: string | null }>(rows: T[]) => rows.filter((row) => row.rapport_id !== null && reportIds.has(row.rapport_id));
  ...
  const formationStats = dataset.formationStats.filter((row) => row.formation_id !== null && formationIds.has(row.formation_id));
  const festivalStats = dataset.festivalStats.filter((row) => row.festival_id !== null && festivalIds.has(row.festival_id));
  const insertionStats = dataset.insertionStats.filter((row) => row.activite_id !== null && insertionIds.has(row.activite_id));
  const demographie = directionId ? dataset.demographie.filter((row) => row.direction_id === directionId) : dataset.demographie;
  const etablissements = directionId ? dataset.etablissements.filter((row) => row.direction_id === directionId) : dataset.etablissements;
  const totalActivites = activites.reduce((total, row) => total + sum([row.activites_culturelles, row.activites_educatives, row.activites_sportives, row.renforcement_capacites]), 0);
  const hommes = sum(participants.map((row) => row.hommes)) + sum(formationStats.map((row) => row.nombre_beneficiaires_hommes)) + sum(festivalStats.map((row) => row.nombre_hommes)) + sum(insertionStats.map((row) => row.hommes));
  const femmes = sum(participants.map((row) => row.femmes)) + sum(formationStats.map((row) => row.nombre_beneficiaires_femmes)) + sum(festivalStats.map((row) => row.nombre_femmes)) + sum(insertionStats.map((row) => row.femmes));
  const totalBeneficiaires = hommes + femmes;
  const totalZones = sum(participants.map((row) => (row.milieu_rural ?? 0) + (row.milieu_urbain ?? 0))) + sum(festivalStats.map((row) => (row.nbr_rural ?? 0) + (row.nbr_urbain ?? 0))) + sum(insertionStats.map((row) => (row.nbr_rural ?? 0) + (row.nbr_urbain ?? 0)));
  const regionalPopulation = sum(demographie.map((row) => row.population_jeune));
  const latestProjects = new Map<string, SuiviProjetRow>();
  projets.forEach((project) => { if (project.etablissement_id) latestProjects.set(project.etablissement_id, project); });
  const blocked = new Set(Array.from(latestProjects.entries()).filter(([, project]) => project.statut === "en_cours" || project.statut === "ferme").map(([id]) => id));
  const activeEstablishments = etablissements.filter((row) => row.type_etablissement === "maison_jeunes" && !blocked.has(row.id)).length;
  const kpis = {
  ...
  const section3 = {
    domaine_educatif: sum(activites.map((row) => row.activites_educatives)), domaine_culturel: sum(activites.map((row) => row.activites_culturelles)), domaine_sportif: sum(activites.map((row) => row.activites_sportives)), domaine_capacite: sum(activites.map((row) => row.renforcement_capacites)), femmes, hommes, rural: sum(participants.map((row) => row.milieu_rural)) + sum(festivalStats.map((row) => row.nbr_rural)) + sum(insertionStats.map((row) => row.nbr_rural)), urbain: sum(participants.map((row) => row.milieu_urbain)) + sum(festivalStats.map((row) => row.nbr_urbain)) + sum(insertionStats.map((row) => row.nbr_urbain)),
  };
  const quarter = (trimestre: string) => scopedReports.filter((rapport) => rapport.trimestre?.toUpperCase() === trimestre);
  const activityEvolution = ["T1", "T2", "T3", "T4"].map((trimestre) => ({ trimestre: trimestre.toLowerCase(), total_activites: quarter(trimestre).length ? totalActivites : null }));
  const establishmentEvolution = ["T1", "T2", "T3", "T4"].map((trimestre) => ({ trimestre: trimestre.toLowerCase(), fonctionnels: quarter(trimestre).length ? activeEstablishments : null, travaux: null, fermes: null }));
...
export async function loadJeunesseRegionalDashboard(
  ...
  const [rapportsResult, dirs] = await Promise.all([
    supabase.from("rapports").select("id, direction_id, annee, statut_rapport, trimestre").eq("annee", year),
    supabase.from("directions").select("*"),
  ]);

  const rapports = (rapportsResult.data ?? []) as JeunesseRapportRow[];
  const dataset = await loadJeunesseDataset(rapports.map((rapport) => rapport.id), year);
  ...
  const dirsWithScores = directionAggregates.map(({ direction: pref, aggregate }) => {
    ...
    const status: RegionalStatus = directionRapports.length === 0 || directionRapports.every((rapport) => rapport.statut_rapport === "NON_COMMENCE")
      ? "NON_COMMENCE"
  ...
  dirsWithScores.forEach((direction, index) => {
    ...
    rankableIndex += 1;
    direction.rang_regional = index > 0 && direction.score === dirsWithScores[index - 1].score
  ...
  if (regionalAggregate.activityEvolution.length > 0) {
    ...
    activityEvolution = trimestresVides.map((tLabel) => {
      ...
      const row = regionalAggregate.activityEvolution.find((r) => r.trimestre === tLabel.toLowerCase());
    ...
    establishmentEvolution = trimestresVides.map((tLabel) => {
      const row = regionalAggregate.establishmentEvolution.find((r) => r.trimestre === tLabel.toLowerCase());
    ...
    activityEvolution = EMPTY_JEUNESSE_TRIMESTRES.map((trimestre) => ({ trimestre, total_activites: null }));
    establishmentEvolution = EMPTY_JEUNESSE_TRIMESTRES.map((trimestre) => ({ trimestre, fonctionnels: null, travaux: null, fermes: null }));
  }

  const completedDirections = dirsWithScores.filter((direction) => direction.statut === "TERMINE").length;
  const inProgressDirections = dirsWithScores.filter((direction) => direction.statut === "EN_COURS").length;
  ...
  return {
    ...
    comparison: {
      ...
      primary: { key: "total_activites", label: "Activités", regionalAverage: dirsWithScores.length ? kpis.total_activites / dirsWithScores.length : null },
      secondary: { key: "total_beneficiaires", label: "Bénéficiaires", regionalAverage: dirsWithScores.length ? kpis.total_beneficiaires / dirsWithScores.length : null },
      score: {
        label: "Score Jeunesse",
        methodology: "Pondération existante : activités 20 %, bénéficiaires 20 %, couverture 15 %, féminisation 15 %, partenariats 15 %, établissements actifs 15 %. Score relatif à l'agrégat régional, borné 0–100 ; NON_COMMENCE = 0.",
      },
    },
  };
}
