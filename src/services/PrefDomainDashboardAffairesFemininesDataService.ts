...
export interface AffairesFemininesDashboardData {
  status: {
  ...
  section3: AffairesFemininesSection3Data;
  evolution: AffairesFemininesEvolutionData;
  benchmark: PrefDomainBenchmarkRow[];
  detailed: AffairesFemininesSection6Data;
}
...
const buildBenchmark = (kpis: AffairesFemininesKpisRaw): PrefDomainBenchmarkRow[] => {
  // Aucune vue régionale n'existe encore pour le domaine Affaires Féminines
  // (même situation que le domaine Infrastructure) : la moyenne régionale
  // est donc à 0 en attendant qu'une vue dédiée soit créée côté base de
  // données. Les scores de la préfecture, eux, sont réels.
  return [
    {
...
benchmark: buildBenchmark(emptyKpis),
  ...
  // 3. Charger le statut (scopé au domaine AF) + les 11 tables af_* (+ secteurs/filieres) en parallèle
  const [
    status,
    inscriptionsClubs,
    inscriptionsOfppt,
    ...
    loadInscriptionsClubs(rapportIds),
    loadInscriptionsOfppt(rapportIds),
    loadActivitesSensibilisation(rapportIds),
    loadPortesOuvertes(rapportIds),
    loadFormationCadres(rapportIds),
    loadMiseAJourReseau(rapportIds),
    loadRessourcesHumaines(rapportIds),
    loadIntegrationLaureates(rapportIds),
    loadActivitesGeneratricesRevenus(rapportIds),
    loadCentresEcoute(rapportIds),
    loadSuiviPartenariats(rapportIds),
    loadSecteurs(),
    loadFilieres(),
  ]);
