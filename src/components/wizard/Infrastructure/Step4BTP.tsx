          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {isAr
                  ? "تتبع تقدم الأشغال والأداء المالي للمشاريع"
                  : "Suivi de l'avancement physique et financier des projets"}
              </p>
            </div>
          </div>
          <Button
          ...
          <div className="space-y-4 pt-2">
            {projets.map((proj, pIdx) => {
              ...
              return (
                <div
                  key={proj.local_id}
                  className="border border-border rounded-lg p-4 bg-muted/20 space-y-4 transition-colors hover:border-primary/30"
                >
                  <div className="flex items-center justify-between gap-2 border-b border-border/40 pb-3">
                    ...
                    <Button
                      ...
                      size="icon"
                      variant="ghost"
                      onClick={() => handleRemoveProjet(proj.local_id)}
                      disabled={disabled}
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    <div className="space-y-1.5">
                      <Label className="text-xs font-medium">
                        {isAr ? "طبيعة المشروع" : "Nature du projet"}
                      </Label>
                      <Select
                        value={proj.type_projet}
                        disabled={disabled}
                        onValueChange={(v) => handleUpdateProjet(proj.local_id, { type_projet: v })}
                      >
                        <SelectTrigger className="h-9 bg-background">
                          <SelectValue
                            placeholder={isAr ? "اختر طبيعة المشروع" : "Choisir la nature"}
                          />
                        </SelectTrigger>
                        <SelectContent>
                          {TYPES_PROJET.map((tp) => (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-border/40 pt-4">
                    <div className="space-y-1.5">
                      ...
                      <Select
                        ...
                        disabled={disabled}
                        onValueChange={(v) =>
                          handleUpdateProjet(proj.local_id, {
                            type_filtre: v,
                            etablissement_id: "",
                          })
                        }
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Select
                        ...
                        onValueChange={(v) =>
                          handleUpdateProjet(proj.local_id, { etablissement_id: v })
                        }
                      >
                        <SelectTrigger className="h-9 bg-background">
                          <SelectValue
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-border/40 pt-4">
                    <div className="space-y-1.5">
                      <NumericField
                        label={
                          isAr ? "الكلفة المالية للمشروع (درهم)" : "Coût financier du projet (DH)"
                        }
                        value={proj.cout_projet}
                        onChange={(val) => handleUpdateProjet(proj.local_id, { cout_projet: val })}
                        disabled={disabled}
                      />
                    </div>
                    <div className="space-y-1.5">
                      <NumericField
                        label={isAr ? "المبلغ المؤدى (درهم)" : "Montant payé (DH)"}
                        value={proj.montant_paye}
                        onChange={(val) => handleUpdateProjet(proj.local_id, { montant_paye: val })}
                        disabled={disabled}
                      />
                    </div>
                    ...
                    <div className="space-y-1.5">
                      <NumericField
                        label={isAr ? "نسبة تقدم الأشغال (%)" : "Avancement des travaux (%)"}
                        value={proj.taux_avancement_travaux}
                        onChange={(val) =>
                          handleUpdateProjet(proj.local_id, { taux_avancement_travaux: val })
                        }
                        disabled={disabled}
                      />
                    </div>
                  ...
                  <div className="pt-4 border-t border-border/40">
                    <div className="space-y-1.5">
                      ...
                      <SafeTextarea
                        placeholder={
                          isAr ? "أضف ملاحظات (اختياري)..." : "Ajouter des observations..."
                        }
                        value={proj.observations || ""}
                        onValueChange={(val) =>
                          handleUpdateProjet(proj.local_id, { observations: val })
                        }
                        disabled={disabled}
                      />
                    </div>
