...
export const Step1Depenses = memo(({ disabled, onActivity, rapportId }: StepComponentProps & { rapportId?: string | null }) => {
  const { i18n } = useTranslation();
  const isAr = i18n.language === 'ar';

  // Liaison avec le Hook
  const { items, add: addEntry, remove: removeEntry, update: updateEntry, loading } = useInfraDepenses(rapportId || null);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold">
                {isAr ? 'نفقات التسيير والاستثمار' : 'Dépenses de Fonctionnement et Investissement'}
              </h2>
              <p className="text-sm text-muted-foreground">
                {isAr ? 'تدبير الاعتمادات المالية والمشاريع الميزانياتية' : 'Gestion des crédits financiers par projet budgétaire'}
              </p>
            </div>
          </div>
          <Button 
              ...
              <div key={item.local_id} className="border border-border rounded-xl p-4 bg-muted/5 space-y-4 transition-colors hover:border-primary/30">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold">{isAr ? 'نوع النفقة' : 'Type de dépense'}</Label>
                    <Select 
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="fonctionnement">{isAr ? 'نفقات التسيير' : 'Fonctionnement'}</SelectItem>
                        <SelectItem value="investissement">{isAr ? 'نفقات الاستثمار' : 'Investissement'}</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
