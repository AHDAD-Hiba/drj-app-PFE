...
export const Step4LiberteSurveillee = memo(({ disabled, onActivity, rapportId }: StepComponentProps & { rapportId?: string | null }) => {
  const { i18n } = useTranslation();
  const isAr = i18n.language === 'ar';

  const { items: statsItems, add: addStat, update: updateStat } = usePeStatistiquesLS(rapportId);
  const { items: rapportItems, add: addRapport, update: updateRapport } = usePeRapportsJudiciaires(rapportId);
      ...
      <Card className="p-5 sm:p-6 bg-card border-border shadow-sm border-t-4 border-t-primary">
        <div className="flex items-center gap-3 border-b border-border pb-4 mb-5">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {isAr ? 'الإحصائيات، الإدماج، والتقارير القضائية' : 'Statistiques, Intégration et Rapports judiciaires'}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <Label className="text-sm font-bold flex items-center gap-1.5 text-primary">
            ...
            {isAr ? '1. الإحصائيات العامة للمستفيدين من الحرية المحروسة' : '1. Statistiques Globales des Bénéficiaires'}
          </Label>
        ...
        <div className="space-y-6">
          <div className="border-b border-border pb-4">
            <Label className="text-sm font-bold flex items-center gap-1.5 text-primary">
              ...
              {isAr ? '2. إدماج الأطفال المحالين على نظام الحرية المحروسة' : '2. Intégration des enfants'}
            </Label>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            />
            <NumericField 
              label={isAr ? 'المندمجين في الأنشطة المستدامة (جمعيات/نوادي)' : 'Activités durables (Clubs)'} 
        ...
        <div className="space-y-6">
          <div className="border-b border-border pb-4">
            <Label className="text-sm font-bold flex items-center gap-1.5 text-primary">
              ...
              {isAr ? '3. عدد التقارير المسلمة للسلطات القضائية' : '3. Rapports soumis à la justice'}
            </Label>
          </div>
          ...
          <div className="flex items-center justify-between p-4 border border-border rounded-lg bg-muted/5 w-full md:w-2/3 lg:w-1/2">
            <Label className="font-semibold">
              {isAr ? 'التقارير المنجزة من طرف مندوبي الحرية المحروسة' : 'Rapports réalisés par les délégués à la LS'}
            </Label>
            <div className="w-32">
              <NumericField 
                label="" 
