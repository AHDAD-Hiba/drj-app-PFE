...
export const Step2Animation = memo(
  ({ disabled, onActivity, rapportId }: StepComponentProps & { rapportId?: string | null }) => {
    ...
    useEffect(() => {
      const fetchRefs = async () => {
        const { data: domaines } = await supabase
          .from("ref_domaines_activite")
          .select("*")
          .order("ordre_affichage");
        if (domaines) setRefDomaines(domaines);

        const { data: incidents } = await supabase
          .from("ref_types_incident")
          .select("*")
          .order("ordre_affichage");
    ...
    const {
      ...
      update: updateAct,
      remove: removeAct,
    } = usePeActivites(rapportId);
    const {
      items: conseils,
      add: addConseil,
      update: updateConseil,
      remove: removeConseil,
    } = usePeConseilEnfant(rapportId);
    ...
    useEffect(() => {
      ...
      conseils.forEach((c) => {
        if (c.etablissement_id) allEtabIds.add(c.etablissement_id);
      });
      dons.forEach((d) => {
        if (d.etablissement_id) allEtabIds.add(d.etablissement_id);
      });
    ...
    const getCenterActivites = (etabId: string) =>
      activites.filter((a) => a.etablissement_id === etabId);
    ...
    const handleAddData = useCallback(
      (addFn: Function, etabId: string, initialData = {}) => {
    ...
    const handleUpdateEtablissement = useCallback(
      (local_id: string, new_etab_id: string) => {
        setCenters((prev) =>
          ...
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold">
                    {isAr
                      ? "إجمالي البرامج والأنشطة المنجزة في مختلف المجالات"
                      : "Activités Provinciales Globales"}
                  </h2>
                  <p className="text-sm text-muted-foreground">
                    {isAr
                      ? "الترفيهية، الثقافية، الرياضية، التربية الصحية والنفسية..."
              ...
              <Button
                type="button"
                size="sm"
                onClick={handleAddGlobalActivite}
                disabled={disabled || refDomaines.length === 0}
                className="gap-1.5"
              >
              ...
              {globalActivitesGenerales.map((act) => (
                ...
                >
                  <div className="md:col-span-3 space-y-1.5">
                    ...
                    <Select
                      value={act.domaine_id}
                      disabled={disabled}
                      onValueChange={(v) => {
                        updateAct(act.local_id, { domaine_id: v });
                        if (onActivity) void onActivity();
                      }}
                    >
                    ...
                    {act.domaine_id === domaineAutreId && (
                      <SafeInput
                        placeholder={isAr ? "يرجى التحديد..." : "Préciser..."}
                        value={act.domaine_autre || ""}
                        onValueChange={(val) => {
                          updateAct(act.local_id, { domaine_autre: val });
                          if (onActivity) void onActivity();
                        }}
                        disabled={disabled}
                        className="h-9 mt-2 text-xs bg-background"
                      />
                    )}
                  </div>
                  <div className="md:col-span-6 space-y-1.5">
                    ...
                    <SafeInput
                      value={act.nom_activite}
                      onValueChange={(val) => {
                        updateAct(act.local_id, { nom_activite: val });
                        if (onActivity) void onActivity();
                      }}
                      disabled={disabled}
                  ...
                  <div className="md:col-span-1">
                    <NumericField
                      ...
                      value={act.nombre_beneficiaires}
                      onChange={(v) => {
                        updateAct(act.local_id, { nombre_beneficiaires: v });
                        if (onActivity) void onActivity();
                      }}
                      disabled={disabled}
                    />
          ...
          <div className="space-y-4 pt-4 border-t border-border">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-emerald-900 dark:text-emerald-400">
                    {isAr
                      ? "أنشطة وبرامج نشر ثقافة حقوق الطفل"
                      : "Activités de Promotion des Droits de l'Enfant"}
                  </h2>
                  <p className="text-sm text-muted-foreground">
                    {isAr
                      ? "عدد الأنشطة والبرامج التي تم تنفيذها لنشر ثقافة حقوق الطفل"
              ...
              <Button
                type="button"
                size="sm"
                onClick={handleAddDroitsEnfantActivite}
                disabled={disabled || !droitsEnfantDomaineId}
                className="gap-1.5"
              >
                <Plus className="h-4 w-4" />
            ...
            <div className="space-y-3">
                ...
                >
                  <div className="md:col-span-5 space-y-1.5">
                    ...
                    <SafeInput
                      ...
                      placeholder={
                        isAr
                          ? "مثال: ورشة تحسيسية حول حقوق الطفل..."
                          : "Ex: Atelier de sensibilisation..."
                      }
                    />
                  </div>
                  <div className="md:col-span-5 space-y-1.5">
                    ...
                    <SafeInput
                      value={act.partenaires || ""}
                      onValueChange={(val) => {
                        updateAct(act.local_id, { partenaires: val });
                        if (onActivity) void onActivity();
                      }}
                      disabled={disabled}
                      className="h-9 bg-background text-xs"
              ...
              {globalActivitesDroitsEnfant.length === 0 && (
                <div className="text-center py-4 text-xs text-muted-foreground border border-dashed border-emerald-500/30 rounded-lg">
                  {isAr ? "لا توجد أنشطة حقوق الطفل مسجلة" : "Aucune activité enregistrée"}
                </div>
              )}
            </div>
          </div>
        ...
        <Card className="p-5 sm:p-6 bg-card border-border shadow-sm border-t-4 border-t-destructive">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border pb-4 mb-5">
            ...
            <Button
              ...
              size="sm"
              variant="outline"
              onClick={handleAddGlobalIncident}
              disabled={disabled || refIncidents.length === 0}
          ...
          <div className="space-y-3">
            {incidents.map((inc) => (
              <div
                key={inc.local_id}
                className="grid grid-cols-1 md:grid-cols-12 gap-3 border border-destructive/20 p-3 rounded-lg bg-destructive/5 items-start"
              >
                <div className="md:col-span-3 space-y-1.5">
                  <Label className="text-xs">{isAr ? "نوع الحادث" : "Type incident"}</Label>
                  <Select
                    value={inc.type_incident_id}
                    disabled={disabled}
                    onValueChange={(v) => {
                      updateIncident(inc.local_id, { type_incident_id: v });
                      if (onActivity) void onActivity();
                    }}
                  >
                    <SelectTrigger className="h-9 bg-background text-xs">
                    ...
                    <SelectContent>
                      {refIncidents.map((r) => (
                        <SelectItem key={r.id} value={r.id}>
                          {isAr ? r.libelle_ar : r.libelle_fr}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                  {inc.type_incident_id === incidentAutreId && (
                    <SafeInput
                      placeholder={isAr ? "يرجى التحديد..." : "Préciser..."}
                      value={inc.type_incident_autre || ""}
                      onValueChange={(val) => {
                        updateIncident(inc.local_id, { type_incident_autre: val });
                        if (onActivity) void onActivity();
                      }}
                      disabled={disabled}
                      className="h-9 mt-2 text-xs bg-background"
                ...
                <div className="md:col-span-4 space-y-1.5">
                  <Label className="text-xs">
                    {isAr ? "موضوع التقرير بالتفصيل" : "Sujet détaillé"}
                  </Label>
                  <SafeTextarea
                    value={inc.sujet_detaille}
                    onValueChange={(val) => {
                      updateIncident(inc.local_id, { sujet_detaille: val });
                      if (onActivity) void onActivity();
                    }}
                    disabled={disabled}
                    placeholder={isAr ? "الموضوع بالتفصيل..." : "Sujet détaillé..."}
                  />
                </div>
                <div className="md:col-span-4 space-y-1.5">
                  <Label className="text-xs">{isAr ? "ملاحظات وتدخلات" : "Observations"}</Label>
                  <SafeTextarea
                    value={inc.observations || ""}
                    onValueChange={(val) => {
                      updateIncident(inc.local_id, { observations: val });
                      if (onActivity) void onActivity();
                    }}
                    disabled={disabled}
                ...
                <div className="md:col-span-1 flex flex-col justify-between h-full">
                  <NumericField
                    label={isAr ? "العدد" : "Cas"}
                    value={inc.nombre_cas}
                    onChange={(v) => {
                      updateIncident(inc.local_id, { nombre_cas: v });
                      if (onActivity) void onActivity();
                    }}
                    disabled={disabled}
                  />
                  <Button
                    ...
                    variant="ghost"
                    onClick={() => {
                      removeIncident(inc.local_id);
                      if (onActivity) void onActivity();
                    }}
                    disabled={disabled}
                    className="h-9 w-9 text-destructive self-end mt-2"
                  >
                    <Trash2 className="h-4 w-4" />
                  </Button>
                </div>
            ...
            {incidents.length === 0 && (
              <div className="text-center py-4 text-xs text-muted-foreground border border-dashed border-destructive/30 rounded-lg">
                {isAr ? "لا توجد تقارير مسجلة" : "Aucun rapport enregistré"}
              </div>
            )}
          </div>
        </Card>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {isAr
                    ? "المهارات الحياتية، مجلس الطفل، والهبات"
                    : "Compétences de vie, conseil de l'enfant, dons"}
                </p>
              </div>
            </div>
          ...
          {loadingEtab ? (
            ...
            <div className="space-y-6 pt-6">
              {centers.map((center, idx) => {
                ...
                const conseilsDuCentre = conseils.filter(
                  (c) => c.etablissement_id === center.etablissement_id,
                );
                const donsDuCentre = dons.filter(
                  (d) => d.etablissement_id === center.etablissement_id,
                );
                    ...
                    <div className="flex flex-col md:flex-row gap-4 items-start md:items-end border-b border-border pb-4">
                      <div className="space-y-1.5 flex-1">
                        ...
                        <Select
                          ...
                          disabled={disabled}
                          onValueChange={(v) => handleUpdateEtablissement(center.local_id, v)}
                        >
                      ...
                      <Button
                        type="button"
                        variant="outline"
                        onClick={() => handleRemoveCentre(center.local_id)}
                        disabled={disabled}
                        ...
                        <div className="space-y-4">
                          <div className="flex items-center justify-between">
                            <Label className="text-sm font-bold flex items-center gap-1.5 text-primary">
                              ...
                              {isAr
                                ? "1. برامج المهارات الحياتية"
                                : "1. Programmes Compétences de vie"}
                            </Label>
                            <Button
                              type="button"
                              size="sm"
                              variant="outline"
                              onClick={() => handleAddCenterActivite(center.etablissement_id)}
                              disabled={disabled || refDomaines.length === 0}
                              className="h-8 gap-1 text-xs"
                            >
                              <Plus className="h-3.5 w-3.5" />{" "}
                              {isAr ? "إضافة برنامج" : "Ajouter un programme"}
                            </Button>
                          </div>
                          ...
                          <div className="space-y-3">
                            {activitesDuCentre.map((act) => (
                              ...
                              >
                                <div className="md:col-span-3 space-y-1.5">
                                  ...
                                  <SafeInput
                                    value={isAr ? "المهارات الحياتية" : "Compétences de vie"}
                                    disabled={true}
                                ...
                                <div className="md:col-span-5 space-y-1.5">
                                  <Label className="text-xs">
                                    {isAr ? "اسم البرنامج" : "Nom du programme"}
                                  </Label>
                                  <SafeInput
                                    value={act.nom_activite}
                                    onValueChange={(val) => {
                            ...
                            {activitesDuCentre.length === 0 && (
                              <div className="text-center py-4 text-xs text-muted-foreground border border-dashed rounded-lg">
                                {isAr ? "لا توجد برامج مسجلة" : "Aucun programme"}
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="space-y-4">
                          <div className="flex items-center justify-between">
                            ...
                            <Button
                              ...
                              variant="outline"
                              onClick={() =>
                                handleAddData(addConseil, center.etablissement_id, {
                                  nom_session: "",
                                  date_session: "",
                                })
                              }
                            ...
                            >
                              <Plus className="h-3.5 w-3.5" />{" "}
                              {isAr ? "إضافة دورة" : "Ajouter une session"}
                            </Button>
                          </div>
                          ...
                          <div className="space-y-3">
                            {conseilsDuCentre.map((cons) => (
                              <div
                                key={cons.local_id}
                                className="flex flex-col md:flex-row gap-3 border border-border p-3 rounded-lg bg-muted/5 items-center"
                              >
                                <div className="flex-1 w-full space-y-1.5">
                                  <Label className="text-xs">
                                    {isAr ? "اسم الدورة" : "Nom de la session"}
                                  </Label>
                                  <SafeInput
                                    value={cons.nom_session}
                                    onValueChange={(val) => {
                                      updateConseil(cons.local_id, { nom_session: val });
                                      if (onActivity) void onActivity();
                                    }}
                                    disabled={disabled}
                                    className="h-9 bg-background text-xs"
                                ...
                                <div className="flex-1 w-full space-y-1.5">
                                  <Label className="text-xs">
                                    {isAr ? "تاريخ الانعقاد" : "Date de la session"}
                                  </Label>
                                  <SafeInput
                                    type="date"
                                    value={cons.date_session}
                                    onValueChange={(val) => {
                                      updateConseil(cons.local_id, { date_session: val });
                                      if (onActivity) void onActivity();
                                    }}
                                    disabled={disabled}
                                    className="h-9 bg-background text-xs"
                                ...
                                <Button
                                  ...
                                  variant="ghost"
                                  onClick={() => {
                                    removeConseil(cons.local_id);
                                    if (onActivity) void onActivity();
                                  }}
                                  disabled={disabled}
                                  className="h-9 w-9 text-destructive mt-6 shrink-0"
                                >
                                  <Trash2 className="h-4 w-4" />
                                </Button>
                              </div>
                        ...
                        <div className="space-y-4">
                          <div className="flex items-center justify-between">
                            ...
                            <Button
                              ...
                              variant="outline"
                              onClick={() =>
                                handleAddData(addDon, center.etablissement_id, {
                                  donateur: "",
                                  nature_don: "",
                                  date_reception: "",
                                  beneficiaires: 0,
                          ...
                          <div className="space-y-3">
                            {donsDuCentre.map((don) => (
                              ...
                              >
                                <div className="md:col-span-3 space-y-1.5">
                                  <Label className="text-xs">
                                    {isAr ? "الجهة المانحة" : "Donateur"}
                                  </Label>
                                  <SafeInput
                                    value={don.donateur}
                                    onValueChange={(val) => {
                                      updateDon(don.local_id, { donateur: val });
                                      if (onActivity) void onActivity();
                                    }}
                                    disabled={disabled}
                                    className="h-9 bg-background text-xs"
                                ...
                                <div className="md:col-span-3 space-y-1.5">
                                  <Label className="text-xs">
                                    {isAr ? "نوع الهبة" : "Nature du don"}
                                  </Label>
                                  <SafeInput
                                    value={don.nature_don}
                                    onValueChange={(val) => {
                                      updateDon(don.local_id, { nature_don: val });
                                      if (onActivity) void onActivity();
                                    }}
                                    disabled={disabled}
                                    className="h-9 bg-background text-xs"
                                ...
                                <div className="md:col-span-3 space-y-1.5">
                                  ...
                                  <SafeInput
                                    type="date"
                                    value={don.date_reception}
                                    onValueChange={(val) => {
                                      updateDon(don.local_id, { date_reception: val });
                                      if (onActivity) void onActivity();
                                    }}
                                    disabled={disabled}
                                    className="h-9 bg-background text-xs"
                                ...
                                <div className="md:col-span-2">
                                  <NumericField
                                    label={isAr ? "المستفيدين" : "Bénéficiaires"}
                                    value={don.beneficiaires}
                                    onChange={(v) => {
                                      updateDon(don.local_id, { beneficiaires: v });
                                      if (onActivity) void onActivity();
                                    }}
                                    disabled={disabled}
                                  />
                                ...
                                <div className="md:col-span-12 space-y-1.5 pt-2 border-t border-border/50 mt-1">
                                  ...
                                  <SafeTextarea
                                    ...
                                    onValueChange={(val) => {
                                      updateDon(don.local_id, { observations: val });
                                      if (onActivity) void onActivity();
                                    }}
                                    disabled={disabled}
                                    placeholder={
                            ...
                            {donsDuCentre.length === 0 && (
                              <div className="text-center py-4 text-xs text-muted-foreground border border-dashed rounded-lg">
                                {isAr ? "لا توجد هبات مسجلة" : "Aucun don enregistré"}
                              </div>
                            )}
                          </div>
                        </div>
