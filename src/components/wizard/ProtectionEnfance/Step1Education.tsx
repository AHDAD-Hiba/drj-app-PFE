...
export const Step1Education = memo(
  ({ disabled, onActivity, rapportId }: StepComponentProps & { rapportId?: string | null }) => {
    ...
    const isDataLoading =
      loadingEtab || demHook.loading || edHook.loading || atHook.loading || foHook.loading;
    ...
    useEffect(() => {
      if (!isDataLoading && !initialized && rapportId) {
        ...
        demHook.items.forEach((i) => {
          if (i.etablissement_id) uniqueIds.add(i.etablissement_id);
        });
        edHook.items.forEach((i) => {
          if (i.etablissement_id) uniqueIds.add(i.etablissement_id);
        });
        atHook.items.forEach((i) => {
          if (i.etablissement_id) uniqueIds.add(i.etablissement_id);
        });
        foHook.items.forEach((i) => {
          if (i.etablissement_id) uniqueIds.add(i.etablissement_id);
        });
    ...
    const handleRemoveCentre = useCallback(
      (local_id: string, etablissement_id: string) => {
        setCenters((prev) => prev.filter((c) => c.local_id !== local_id));
        if (etablissement_id) {
          demHook.items
            .filter((i) => i.etablissement_id === etablissement_id)
            .forEach((i) => void demHook.remove(i.local_id));
          edHook.items
            .filter((i) => i.etablissement_id === etablissement_id)
            .forEach((i) => void edHook.remove(i.local_id));
          atHook.items
            .filter((i) => i.etablissement_id === etablissement_id)
            .forEach((i) => void atHook.remove(i.local_id));
          foHook.items
            .filter((i) => i.etablissement_id === etablissement_id)
            .forEach((i) => void foHook.remove(i.local_id));
        }
        if (onActivity) onActivity();
      },
    ...
    const handleUpdateEtablissement = useCallback(
      (local_id: string, old_etab_id: string, new_etab_id: string) => {
        setCenters((prev) =>
          prev.map((c) => (c.local_id === local_id ? { ...c, etablissement_id: new_etab_id } : c)),
        );
        if (old_etab_id) {
          demHook.items
            .filter((i) => i.etablissement_id === old_etab_id)
            .forEach((i) => void demHook.update(i.local_id, { etablissement_id: new_etab_id }));
          edHook.items
            .filter((i) => i.etablissement_id === old_etab_id)
            .forEach((i) => void edHook.update(i.local_id, { etablissement_id: new_etab_id }));
          atHook.items
            .filter((i) => i.etablissement_id === old_etab_id)
            .forEach((i) => void atHook.update(i.local_id, { etablissement_id: new_etab_id }));
          foHook.items
            .filter((i) => i.etablissement_id === old_etab_id)
            .forEach((i) => void foHook.update(i.local_id, { etablissement_id: new_etab_id }));
        }
        if (onActivity) onActivity();
      },
      [demHook, edHook, atHook, foHook, onActivity],
    ...
    const globalDemEntry = demHook.items.find(
      (i) => !i.etablissement_id && i.type_prise_charge === "centre_sauvegarde",
    );
    const handleGlobalDemChange = (field: string, value: number) => {
      if (globalDemEntry) {
        ...
        void demHook.add({
          ...
          type_prise_charge: "centre_sauvegarde",
          garcons: 0,
          filles: 0,
          migrants_non_accompagnes: 0,
          changement_mesure: 0,
          taux_preparation_integration: 0,
          [field]: value,
        } as any);
      }
      if (onActivity) onActivity();
    ...
    const handleCenterDemChange = (etablissement_id: string, value: number) => {
      const existing = demHook.items.find(
        (i) =>
          i.etablissement_id === etablissement_id && i.type_prise_charge === "centre_sauvegarde",
      );
      if (existing) {
    ...
    const handleCenterEdChange = (etablissement_id: string, field: string, value: number) => {
      ...
      if (existing) {
        ...
        void edHook.add({
          local_id: crypto.randomUUID(),
          etablissement_id,
          beneficiaires_formel: 0,
          beneficiaires_non_formel: 0,
          beneficiaires_soutien: 0,
          [field]: value,
        } as any);
      }
      if (onActivity) onActivity();
    ...
    const handleCenterFoChange = (etablissement_id: string, field: string, value: number) => {
      ...
      if (existing) {
        ...
        void foHook.add({
          local_id: crypto.randomUUID(),
          etablissement_id,
          beneficiaires_intra: 0,
          beneficiaires_extra: 0,
          beneficiaires_initiation: 0,
          [field]: value,
        } as any);
      }
      if (onActivity) onActivity();
      ...
      <div className="space-y-8">
        ...
        <Card className="p-5 sm:p-6 bg-card border-border shadow-sm border-t-4 border-t-primary">
          <div className="flex items-center gap-3 border-b border-border pb-4 mb-5">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {isAr
                  ? "تخص جميع مراكز حماية الطفولة التابعة للمديرية"
                  : "Aggrégation provinciale pour l'ensemble des centres"}
              </p>
            </div>
          </div>
          ...
          {isDataLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <NumericField
                ...
                value={globalDemEntry?.garcons || 0}
                onChange={(v) => handleGlobalDemChange("garcons", v)}
                disabled={disabled}
              />
              <NumericField
                label={isAr ? "إجمالي الإناث" : "Total Filles"}
                value={globalDemEntry?.filles || 0}
                onChange={(v) => handleGlobalDemChange("filles", v)}
                disabled={disabled}
              />
              <NumericField
                label={isAr ? "المهاجرين غير المرفقين" : "Migrants non accompagnés"}
                value={globalDemEntry?.migrants_non_accompagnes || 0}
                onChange={(v) => handleGlobalDemChange("migrants_non_accompagnes", v)}
                disabled={disabled}
              />
              <NumericField
                label={isAr ? "نسبة الأطفال المهيئين للاندماج %" : "Taux d'intégration %"}
                value={globalDemEntry?.taux_preparation_integration || 0}
                onChange={(v) => handleGlobalDemChange("taux_preparation_integration", v)}
                disabled={disabled}
              />
            </div>
          )}
          <p className="text-xs text-muted-foreground mt-3">
            *{" "}
            {isAr
              ? "ملاحظة: إحصائيات نظام الحرية المحروسة سيتم إدخالها في المرحلة 4 الخاصة بها."
              : "Note: Les statistiques de la Liberté Surveillée seront saisies dans l'Étape 4."}
          </p>
        </Card>
        ...
        <Card className="p-5 sm:p-6 bg-card border-border shadow-sm">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {isAr
                    ? "تغيير التدبير، التمدرس، والتكوين المهني لكل مركز"
                    : "Changement de mesure, éducation et formation par centre"}
                </p>
              </div>
            </div>
          ...
          {isDataLoading ? (
            ...
            <div className="text-center py-10 text-sm text-muted-foreground border-2 border-dashed border-border rounded-lg bg-muted/5 mt-4">
              {isAr
                ? 'لم يتم تسجيل أي مركز بعد. اضغط على "إضافة مركز" لإدخال بيانات التمدرس والورشات.'
                : 'Aucun centre enregistré. Cliquez sur "Ajouter un centre".'}
            </div>
          ) : (
            <div className="space-y-6 pt-4">
              {centers.map((center, idx) => {
                ...
                const centerDem = demHook.items.find(
                  (i) => i.etablissement_id === center.etablissement_id,
                );
                const centerEd = edHook.items.find(
                  (i) => i.etablissement_id === center.etablissement_id,
                );
                const centerFo = foHook.items.find(
                  (i) => i.etablissement_id === center.etablissement_id,
                );
                const centerAteliers = atHook.items.filter(
                  (i) => i.etablissement_id === center.etablissement_id,
                );
                ...
                return (
                  <div
                    key={center.local_id}
                    className="border border-border rounded-lg p-5 bg-card space-y-6 shadow-sm"
                  >
                    {/* اختيار المركز */}
                      ...
                      <div className="space-y-8 pt-2">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border border-border/60 p-4 rounded-lg bg-muted/5">
                            <div className="md:col-span-1">
                              <NumericField
                                label={
                                  isAr
                                    ? "عدد الأطفال في وضعية صعبة الذين تم تغيير تدبيرهم"
                                    : "Enfants dont la mesure a été changée"
                                }
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border border-border/60 p-4 rounded-lg bg-muted/5">
                            <NumericField
                              label={isAr ? "التعليم النظامي" : "Formel"}
                              value={centerEd?.beneficiaires_formel || 0}
                              onChange={(v) =>
                                handleCenterEdChange(
                                  center.etablissement_id,
                                  "beneficiaires_formel",
                                  v,
                                )
                              }
                              disabled={disabled}
                            />
                            <NumericField
                              label={isAr ? "التعليم غير النظامي" : "Non formel"}
                              value={centerEd?.beneficiaires_non_formel || 0}
                              onChange={(v) =>
                                handleCenterEdChange(
                                  center.etablissement_id,
                                  "beneficiaires_non_formel",
                                  v,
                                )
                              }
                              disabled={disabled}
                            />
                            <NumericField
                              label={isAr ? "الدعم الدراسي" : "Soutien scolaire"}
                              value={centerEd?.beneficiaires_soutien || 0}
                              onChange={(v) =>
                                handleCenterEdChange(
                                  center.etablissement_id,
                                  "beneficiaires_soutien",
                                  v,
                                )
                              }
                              disabled={disabled}
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <Button
                              ...
                              onClick={() => {
                                void atHook.add({
                                  local_id: crypto.randomUUID(),
                                  etablissement_id: center.etablissement_id,
                                  nom_atelier: "",
                                  nombre: 0,
                                });
                                if (onActivity) onActivity();
                            ...
                            >
                              ...
                              {isAr ? "إضافة ورشة" : "Ajouter un atelier"}
                            </Button>
                          </div>
                          ...
                          {centerAteliers.length === 0 ? (
                            ...
                            <div className="space-y-2">
                              {centerAteliers.map((at) => (
                                <div
                                  key={at.local_id}
                                  className="flex items-center gap-3 border border-border p-2.5 rounded-lg bg-muted/10"
                                >
                                  <div className="flex-1">
                                    <SafeInput
                                      placeholder={
                                        isAr ? "اسم الورشة (مثال: الحلاقة)" : "Nom de l'atelier"
                                      }
                                      value={at.nom_atelier}
                                      onValueChange={(value) => {
                                        void atHook.update(at.local_id, { nom_atelier: value });
                                        if (onActivity) onActivity();
                                      }}
                                      disabled={disabled}
                                  ...
                                  <div className="w-28">
                                    <NumericField
                                      ...
                                      value={at.nombre}
                                      onChange={(v) => {
                                        void atHook.update(at.local_id, { nombre: v });
                                        if (onActivity) onActivity();
                                      }}
                                      disabled={disabled}
                                    />
                                  ...
                                  <Button
                                    ...
                                    variant="ghost"
                                    onClick={() => {
                                      void atHook.remove(at.local_id);
                                      if (onActivity) onActivity();
                                    }}
                                    disabled={disabled}
                        ...
                        <div className="space-y-4">
                          <Label className="text-sm font-bold flex items-center gap-1.5 text-primary">
                            ...
                            {isAr
                              ? "4. المستفيدون من التكوين أو التدريب"
                              : "4. Bénéficiaires de la formation"}
                          </Label>
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border border-border/60 p-4 rounded-lg bg-muted/5">
                            <NumericField
                              label={isAr ? "داخل المركز (Intra)" : "Au sein du centre"}
                              value={centerFo?.beneficiaires_intra || 0}
                              onChange={(v) =>
                                handleCenterFoChange(
                                  center.etablissement_id,
                                  "beneficiaires_intra",
                                  v,
                                )
                            ...
                            />
                            <NumericField
                              label={isAr ? "خارج المركز (Extra/OFPPT)" : "En dehors du centre"}
                              value={centerFo?.beneficiaires_extra || 0}
                              onChange={(v) =>
                                handleCenterFoChange(
                                  center.etablissement_id,
                                  "beneficiaires_extra",
                            ...
                            />
                            <NumericField
                              label={isAr ? "الاستئناس المهني" : "Initiation professionnelle"}
                              value={centerFo?.beneficiaires_initiation || 0}
                              onChange={(v) =>
                                handleCenterFoChange(
                                  center.etablissement_id,
                                  "beneficiaires_initiation",
                                  v,
                                )
                              }
                              disabled={disabled}
