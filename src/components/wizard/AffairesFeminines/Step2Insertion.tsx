          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold">
                {isAr ? "إدماج الخريجات" : "Intégration des lauréates"}
              </h2>
              <p className="text-sm text-muted-foreground">
                {isAr ? "تتبع الإدماج المهني للخريجات" : "Suivi de l'insertion professionnelle"}
              </p>
            </div>
          </div>
          <Button
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Button
                    ...
                    variant="ghost"
                    onClick={() => {
                      laureates.remove(item.local_id);
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold">
                      {isAr ? "عدد الخريجات" : "Nombre de lauréates"}
                    </Label>
                    <NumericField
                      label=""
                      value={item.nombre_laureates}
                      onChange={(v) => {
                        laureates.update(item.local_id, { nombre_laureates: v });
                        if (onActivity) void onActivity();
                      }}
                      disabled={disabled}
                    />
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold">
                      {isAr ? "عدد المدمجات" : "Nombre d'intégrées"}
                    </Label>
                    <NumericField
                      label=""
                      value={item.nombre_integrees}
                      onChange={(v) => {
                        laureates.update(item.local_id, { nombre_integrees: v });
                        if (onActivity) void onActivity();
                      }}
                      disabled={disabled}
                    />
      ...
      <Card className="p-5 sm:p-6 space-y-4 bg-card">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold">
                {isAr ? "الأنشطة المدرة للدخل" : "Activités Génératrices de Revenus"}
              </h2>
              <p className="text-sm text-muted-foreground">
                {isAr ? "المشاريع والتعاونيات" : "Projets et coopératives"}
              </p>
            </div>
          </div>
          <Button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div className="space-y-1.5 sm:col-span-2 lg:col-span-3">
                    ...
                    <Select
                      ...
                      onValueChange={(v) => {
                        agrs.update(item.local_id, { etablissement_id: v });
                        if (onActivity) void onActivity();
                      }}
                    >
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold">
                      {isAr ? "عدد المستفيدات" : "Nombre de bénéficiaires"}
                    </Label>
                    <NumericField
                      label=""
                      value={item.nombre_beneficiaires}
                      onChange={(v) => {
                        agrs.update(item.local_id, { nombre_beneficiaires: v });
                        if (onActivity) void onActivity();
                      }}
                      disabled={disabled}
                    />
