...
export const Step6Infra = memo(({ rapportId, disabled, onActivity }: StepComponentProps) => {
  ...
  useEffect(() => {
    ...
    const nouveauxDansCeRapport = reseau.items.filter(
      m => m.type_mise_a_jour === 'nouvel' || m.type_mise_a_jour === 'creation_en_cours' || !m.etablissement_id
    );
    
    const nouveauxIds = new Set(nouveauxDansCeRapport.map(m => m.etablissement_id).filter(Boolean));
    ...
    const baseList: AfMouvementEntry[] = etabsAnciens.map(etab => {
      ...
      if (mouvementExistant) {
        return { ...mouvementExistant, nom_etablissement: etab.nom, type_etablissement: etab.type_etablissement, is_new_entry: false };
      }
      return {
        local_id: etab.id, is_new_entry: false, etablissement_id: etab.id,
        nom_etablissement: etab.nom, type_etablissement: etab.type_etablissement,
        type_mise_a_jour: 'sans_changement', statut_juridique: '', date_mouvement: '', raisons: '', suggestions: '', observations: ''
      };
    });
  ...
  const handleUpdateMouvement = async (local_id: string, patch: Partial<AfMouvementEntry>) => {
    ...
    const wasAlreadyInDB = reseau.items.some(m => m.local_id === local_id || (m.etablissement_id && m.etablissement_id === currentItem.etablissement_id));
    ...
    if (isNowSansChangement && wasAlreadyInDB) {
       ...
       if (dbItem) await reseau.remove(dbItem.local_id);
        ...
        if (!currentItem.id && !wasAlreadyInDB) {
           ...
           const idToUpdate = currentItem.id ? currentItem.local_id : (reseau.items.find(m => m.etablissement_id === currentItem.etablissement_id)?.local_id || local_id);
                      ...
                      <SafeInput 
                        ...
                        onValueChange={(val) => {
                          setLocalMouvements(prev => 
                            prev.map(m => m.local_id === item.local_id ? { ...m, nom_etablissement: val } : m)
                          );
                          if (val.trim() !== '') {
                    ...
                    <>
                      <div className="space-y-1.5">
                        ...
                        <Select 
                          ...
                          disabled={disabled}
                          onValueChange={(v) => handleUpdateMouvement(item.local_id, { type_etablissement: v })}
                        >
                          <SelectTrigger className="h-10 bg-background">
                            <SelectValue placeholder={isAr ? 'اختر النوع' : 'Choisir le type'} />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value="club_feminin">{isAr ? 'نادي نسوي' : 'Club Féminin'}</SelectItem>
                            <SelectItem value="ofppt">{isAr ? 'مكتب التكوين المهني' : 'OFPPT'}</SelectItem>
                          </SelectContent>
                        </Select>
                      </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <SafeInput 
                          ...
                          onValueChange={(val) => handleUpdateMouvement(item.local_id, { statut_juridique: val })} 
                          disabled={disabled} 
                          className="h-10 bg-background" 
                        />
                      </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <SafeInput 
                          ...
                          onValueChange={(val) => handleUpdateMouvement(item.local_id, { date_mouvement: val })} 
                          disabled={disabled} 
                          className="h-10 bg-background" 
                        />
                      </div>
                  ...
                  {item.type_mise_a_jour === 'fermeture_temporaire' && (
                    <>
                      ...
                      <div className="space-y-1.5 sm:col-span-2 lg:col-span-3">
                        ...
                        <SafeTextarea 
                          value={item.raisons} 
                          onValueChange={(val) => handleUpdateMouvement(item.local_id, { raisons: val })} 
                          disabled={disabled} 
                      ...
                      <div className="space-y-1.5 sm:col-span-2 lg:col-span-3">
                        ...
                        <SafeTextarea 
                          ...
                          onValueChange={(val) => handleUpdateMouvement(item.local_id, { suggestions: val })} 
                          disabled={disabled} 
                  ...
                  {item.type_mise_a_jour !== 'sans_changement' && (
                    <div className="space-y-1.5 sm:col-span-2 lg:col-span-3 border-t border-border/50 pt-3 mt-1">
                      ...
                      <SafeTextarea 
                        ...
                        onValueChange={(val) => handleUpdateMouvement(item.local_id, { observations: val })} 
                        disabled={disabled} 
