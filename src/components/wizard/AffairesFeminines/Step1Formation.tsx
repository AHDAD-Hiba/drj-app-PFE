...
export const Step1Formation = memo(({ rapportId, disabled, onActivity }: StepComponentProps) => {
  ...
  const { items: etablissements } = useAfEtablissements(effectiveDirectionId);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {isAr
                  ? "إحصائيات حسب المؤسسة، الشعبة ونوع التكوين"
                  : "Statistiques par établissement, filière et type"}
              </p>
            </div>
          </div>
          <Button
        ...
        {clubs.items.length === 0 ? (
          ...
          <div className="space-y-4 pt-2">
            {clubs.items.map((item, idx) => (
              <div
                key={item.local_id}
                className="border border-border rounded-xl p-4 bg-muted/10 space-y-4 relative group transition-colors hover:border-primary/30"
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                  ...
                  <div className="space-y-1.5 lg:col-span-1">
                    ...
                    <Select
                      ...
                      onValueChange={(v) => {
                        clubs.update(item.local_id, { etablissement_id: v });
                        if (onActivity) onActivity();
                      }}
                    >
                      <SelectTrigger className="h-10">
                        <SelectValue placeholder={isAr ? "اختر المؤسسة" : "Sélectionner"} />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="space-y-1.5 lg:col-span-1">
                    ...
                    <Select
                      ...
                      onValueChange={(v) => {
                        clubs.update(item.local_id, { filiere_id: v });
                        if (onActivity) onActivity();
                      }}
                    >
                      <SelectTrigger className="h-10">
                        <SelectValue placeholder={isAr ? "الشعبة" : "Filière"} />
                      </SelectTrigger>
                      <SelectContent>
                        {filieres.map((f) => (
                          <SelectItem key={f.id} value={f.id}>
                            {isAr ? f.nom_ar : f.nom_fr || f.nom_ar}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  ...
                  <div className="space-y-1.5 lg:col-span-1">
                    <Label className="text-xs font-semibold">
                      {isAr ? "نوع التكوين" : "Type formation"}
                    </Label>
                    <Select
                      disabled={disabled}
                      value={item.type_formation}
                      onValueChange={(v) => {
                        clubs.update(item.local_id, { type_formation: v });
                        if (onActivity) onActivity();
                      }}
                    >
                      <SelectTrigger className="h-10">
                      ...
                      <SelectContent>
                        <SelectItem value="fondamental">
                          {isAr ? "تكوين أساسي" : "Fondamental"}
                        </SelectItem>
                  ...
                  <div className="space-y-1.5 lg:col-span-1">
                    <Label className="text-xs font-semibold">
                      {isAr ? "مسجلات السنة 1" : "Inscrites A1"}
                    </Label>
                    <NumericField
                      label=""
                      value={item.inscrites_annee_1}
                      onChange={(v) => {
                        clubs.update(item.local_id, { inscrites_annee_1: v });
                        if (onActivity) onActivity();
                      }}
                      disabled={disabled}
                    />
                  ...
                  <div className="space-y-1.5 lg:col-span-1">
                    <Label className="text-xs font-semibold">
                      {isAr ? "مسجلات السنة 2" : "Inscrites A2"}
                    </Label>
                    <NumericField
                      label=""
                      value={item.inscrites_annee_2}
                      onChange={(v) => {
                        clubs.update(item.local_id, { inscrites_annee_2: v });
                        if (onActivity) onActivity();
                      }}
                      disabled={disabled}
                    />
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold">
                {isAr ? "مكتب التكوين المهني" : "Inscriptions OFPPT"}
              </h2>
              <p className="text-sm text-muted-foreground">
                {isAr
                  ? "إحصائيات حسب المؤسسة، القطاع والمستوى"
                  : "Statistiques par établissement, secteur et niveau"}
              </p>
            </div>
          </div>
          <Button
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
                    ...
                    <div className="space-y-1.5 lg:col-span-1">
                      ...
                      <Select
                        ...
                        value={item.etablissement_id}
                        onValueChange={(v) => {
                          ofppt.update(item.local_id, { etablissement_id: v });
                          if (onActivity) onActivity();
                        }}
                      >
                        <SelectTrigger className="h-10">
                          <SelectValue placeholder={isAr ? "المؤسسة" : "Établissement"} />
                        </SelectTrigger>
                        <SelectContent>
                          {ofpptEtabs.map((etab) => (
                            <SelectItem key={etab.id} value={etab.id}>
                    ...
                    <div className="space-y-1.5 lg:col-span-1">
                      ...
                      <Select
                        disabled={disabled}
                        value={item.secteur_id}
                        onValueChange={(v) => {
                          ofppt.update(item.local_id, { secteur_id: v, filiere_id: "" });
                          if (onActivity) onActivity();
                        }}
                      >
                        <SelectTrigger className="h-10">
                          <SelectValue placeholder={isAr ? "القطاع" : "Secteur"} />
                        </SelectTrigger>
                        <SelectContent>
                          {secteurs.map((s) => (
                            <SelectItem key={s.id} value={s.id}>
                              {isAr ? s.nom_ar : s.nom_fr || s.nom_ar}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    ...
                    <div className="space-y-1.5 lg:col-span-1">
                      <Label className="text-xs font-semibold">{isAr ? "الشعبة" : "Filière"}</Label>
                      <Select
                        disabled={disabled || !item.secteur_id}
                        value={item.filiere_id}
                        onValueChange={(v) => {
                          ofppt.update(item.local_id, { filiere_id: v });
                          if (onActivity) onActivity();
                        }}
                      >
                        <SelectTrigger className="h-10">
                    ...
                    <div className="space-y-1.5 lg:col-span-1">
                      ...
                      <Select
                        disabled={disabled}
                        value={item.niveau_formation}
                        onValueChange={(v) => {
                          ofppt.update(item.local_id, { niveau_formation: v });
                          if (onActivity) onActivity();
                        }}
                      >
                        <SelectTrigger className="h-10">
                          <SelectValue placeholder={isAr ? "المستوى" : "Niveau"} />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="specialisation">
                            {isAr ? "التخصص" : "Spécialisation"}
                          </SelectItem>
                          <SelectItem value="qualification">
                            {isAr ? "التأهيل" : "Qualification"}
                          </SelectItem>
                          <SelectItem value="technicien">
                            {isAr ? "التقني" : "Technicien"}
                          </SelectItem>
                          <SelectItem value="technicien_specialise">
                            {isAr ? "التقني المتخصص" : "Tech. Spécialisé"}
                          </SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    ...
                    <div className="space-y-1.5 lg:col-span-1">
                      ...
                      <NumericField
                        ...
                        value={item.inscrites_annee_1}
                        onChange={(v) => {
                          ofppt.update(item.local_id, { inscrites_annee_1: v });
                          if (onActivity) onActivity();
                        }}
                        disabled={disabled}
                      />
                    ...
                    <div className="space-y-1.5 lg:col-span-1">
                      ...
                      <NumericField
                        ...
                        value={item.inscrites_annee_2}
                        onChange={(v) => {
                          ofppt.update(item.local_id, { inscrites_annee_2: v });
                          if (onActivity) onActivity();
                        }}
                        disabled={disabled}
                      />
