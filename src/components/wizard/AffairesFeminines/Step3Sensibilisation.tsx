...
export const Step3Sensibilisation = memo(
  ({ rapportId, disabled, onActivity }: StepComponentProps) => {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold">
                  {isAr ? "أنشطة التحسيس والتوعية" : "Activités de sensibilisation"}
                </h2>
                <p className="text-sm text-muted-foreground">
                  {isAr
                    ? "الندوات، الورشات والحملات التحسيسية"
                    : "Conférences, ateliers et campagnes"}
                </p>
              </div>
            </div>
            <Button
              type="button"
              size="sm"
              onClick={handleAddSensibilisation}
              disabled={disabled}
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Button
                      ...
                      variant="ghost"
                      onClick={() => {
                        sensibilisations.remove(item.local_id);
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <Select
                        disabled={disabled}
                        value={item.type_activite_id}
                        onValueChange={(v) => {
                          sensibilisations.update(item.local_id, { type_activite_id: v });
                          if (onActivity) void onActivity();
                        }}
                      >
                        <SelectTrigger className="h-10">
                          <SelectValue placeholder={isAr ? "اختر النوع" : "Sélectionner"} />
                        </SelectTrigger>
                        <SelectContent>
                          {typesActivite.map((type) => (
                            <SelectItem key={type.id} value={type.id}>
                              {isAr ? type.nom_ar : type.nom_fr || type.nom_ar}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-xs font-semibold">
                        {isAr ? "المستفيدين (حضري)" : "Bénéficiaires (Urbain)"}
                      </Label>
                      <NumericField
                        label=""
                        value={item.benef_urbain}
                        onChange={(v) => {
                          sensibilisations.update(item.local_id, { benef_urbain: v });
                          if (onActivity) void onActivity();
                        }}
                        disabled={disabled}
                      />
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-xs font-semibold">
                        {isAr ? "المستفيدين (قروي)" : "Bénéficiaires (Rural)"}
                      </Label>
                      <NumericField
                        label=""
                        value={item.benef_rural}
                        onChange={(v) => {
                          sensibilisations.update(item.local_id, { benef_rural: v });
                          if (onActivity) void onActivity();
                        }}
                        disabled={disabled}
                      />
                    ...
                    <div className="space-y-1.5 sm:col-span-2 lg:col-span-3">
                      <Label className="text-xs font-semibold">
                        {isAr ? "النتائج / التقييم" : "Résultats / Évaluation"}
                      </Label>
                      <SafeTextarea
        ...
        <Card className="p-5 sm:p-6 space-y-4 bg-card">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold">
                  {isAr ? "الأبواب المفتوحة" : "Journées Portes Ouvertes"}
                </h2>
                <p className="text-sm text-muted-foreground">
                  {isAr
                    ? "أنشطة الإشعاع والتعريف بالمؤسسات"
                    : "Rayonnement et présentation des établissements"}
                </p>
              </div>
            </div>
            <Button
              type="button"
              size="sm"
              onClick={handleAddPortesOuvertes}
              disabled={disabled}
              className="gap-1.5 shadow-sm"
            >
              <Plus className="h-4 w-4" />
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Button
                      ...
                      variant="ghost"
                      onClick={() => {
                        portesOuvertes.remove(item.local_id);
                        if (onActivity) void onActivity();
                      }}
                      disabled={disabled}
                      className="h-8 w-8 text-destructive hover:text-destructive hover:bg-destructive/10"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div className="space-y-1.5 sm:col-span-2 lg:col-span-3">
                      <Label className="text-xs font-semibold">
                        {isAr ? "المؤسسة" : "Nom de l'établissement"}
                      </Label>
                      <Select
                        disabled={disabled}
                        value={item.etablissement_id}
                        onValueChange={(v) => {
                          portesOuvertes.update(item.local_id, { etablissement_id: v });
                          if (onActivity) void onActivity();
                        }}
                      >
                        <SelectTrigger className="h-10">
                          <SelectValue
                            placeholder={isAr ? "اختر المؤسسة" : "Sélectionner l'établissement"}
                          />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Select
                        ...
                        value={item.type_activite_id}
                        onValueChange={(v) => {
                          portesOuvertes.update(item.local_id, { type_activite_id: v });
                          if (onActivity) void onActivity();
                        }}
                      >
                        <SelectTrigger className="h-10">
                    ...
                    <div className="space-y-1.5 lg:col-span-2">
                      <Label className="text-xs font-semibold">
                        {isAr ? "مضمون النشاط" : "Contenu de l'activité"}
                      </Label>
                      <SafeInput
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-xs font-semibold">
                        {isAr ? "عدد المستفيدين" : "Nombre de bénéficiaires"}
                      </Label>
                      <NumericField
                        label=""
                        value={item.nombre_beneficiaires}
                        onChange={(v) => {
                          portesOuvertes.update(item.local_id, { nombre_beneficiaires: v });
                          if (onActivity) void onActivity();
                        }}
                        disabled={disabled}
                      />
