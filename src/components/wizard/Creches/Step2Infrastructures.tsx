...
export const Step2Infrastructures = memo(
  ({ disabled, onActivity, rapportId }: StepComponentProps) => {
    ...
    const crechesPubliques = etablissements.filter(
      (e) => e.type_etablissement === "creche_publique",
    );
    ...
    const {
      ...
      add: addStat,
      update: updateStat,
    } = useCrStatsInfrastructures(rapportId || null);
    const {
      items: mouvements,
      add: addMouvement,
      update: updateMouvement,
      remove: removeMouvement,
    } = useCrMouvementsFermetures(rapportId || null);
    const {
      items: partenariats,
      add: addPartenariat,
      update: updatePartenariat,
      remove: removePartenariat,
    } = useCrPartenariats(rapportId || null);
    const {
      items: cadres,
      add: addCadre,
      update: updateCadre,
      remove: removeCadre,
    } = useCrCadresAssermentes(rapportId || null);
    const {
      items: labels,
      add: addLabel,
      update: updateLabel,
      remove: removeLabel,
    } = useCrLabelQualite(rapportId || null);
          ...
          <div className="flex items-center gap-3 border-b border-border pb-4 mb-5">
            ...
            <div>
              <h2 className="text-lg font-bold">
                {isAr ? "1. إحصائيات البنية التحتية" : "1. Statistiques Infrastructures"}
              </h2>
              <p className="text-sm text-muted-foreground">
                {isAr
                  ? "دور الحضانة المحدثة، المؤهلة والمجهزة"
                  : "Crèches créées, qualifiées et équipées"}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-4 border-b border-border pb-4 mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold">
                  {isAr
                    ? "2. حركية المؤسسات (الإغلاق وإعادة الفتح)"
                    : "2. Mouvements des établissements"}
                </h2>
              </div>
            </div>
            <Button
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-4 border-b border-border pb-4 mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold">
                  {isAr
                    ? "3. الاتفاقيات الموقعة وتقييم التزام الشركاء"
                    : "3. Partenariats & Conventions"}
                </h2>
              </div>
            </div>
            <Button
                ...
                <div className="md:col-span-11 grid grid-cols-1 md:grid-cols-12 gap-4">
                  ...
                  <div className="md:col-span-6 space-y-1.5">
                    <Label className="text-xs">
                      {isAr ? "الهدف (تجهيز، تكوين...)" : "Objectif"}
                    </Label>
                    <SafeTextarea
                      value={part.objectif || ""}
                      onValueChange={(val) => {
        ...
        <Card className="p-5 sm:p-6 bg-card border-border shadow-sm">
          <div className="flex flex-col sm:flex-row justify-between gap-4 border-b border-border pb-4 mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold">
                  {isAr ? "4. وضعية الأطر المحلفة" : "4. Cadres Assermentés"}
                </h2>
              </div>
            </div>
            <Button
        ...
        <Card className="p-5 sm:p-6 bg-card border-border shadow-sm">
          <div className="flex flex-col sm:flex-row justify-between gap-4 border-b border-border pb-4 mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold">
                  {isAr
                    ? "5. مؤسسات دور الحضانة المقترحة/الحاصلة على علامة الجودة"
                    : "5. Établissements proposés/obtenant le Label de Qualité"}
                </h2>
                <p className="text-sm text-muted-foreground">
                  {isAr ? "الخاصة والتابعة للقطاع" : "Secteur privé et public"}
                </p>
              </div>
            </div>
            <Button
          ...
          <div className="space-y-3">
            {labels.map((lbl) => (
              ...
              >
                <div className="md:col-span-3 space-y-1.5">
                  ...
                  >
                    <SelectTrigger className="h-9 text-xs bg-background">
                      <SelectValue placeholder={isAr ? "اختر القطاع" : "Secteur"} />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="public">
                        {isAr ? "عمومية (تابعة للقطاع)" : "Public"}
                      </SelectItem>
                ...
                <div className="md:col-span-5 space-y-1.5">
                  ...
                  {lbl.etablissement_id !== null ? (
                    <Select
                      value={lbl.etablissement_id || ""}
                      disabled={disabled}
                      onValueChange={(v) => {
                        updateLabel(lbl.local_id, { etablissement_id: v });
                        if (onActivity) void onActivity();
                      }}
                    >
                      <SelectTrigger className="h-9 text-xs bg-background">
                        <SelectValue
                          placeholder={isAr ? "اختر الحضانة العمومية" : "Crèche publique"}
                        />
                      </SelectTrigger>
                      <SelectContent>
                        {crechesPubliques.map((c) => (
                    ...
                    <Select
                      value={lbl.creche_privee_id || ""}
                      disabled={disabled}
                      onValueChange={(v) => {
                        updateLabel(lbl.local_id, { creche_privee_id: v });
                        if (onActivity) void onActivity();
                      }}
                    >
                      <SelectTrigger className="h-9 text-xs bg-background">
                        <SelectValue placeholder={isAr ? "اختر الحضانة الخاصة" : "Crèche privée"} />
                      </SelectTrigger>
                      <SelectContent>
                ...
                <div className="md:col-span-3 space-y-1.5">
                  ...
                  <Select
                    value={lbl.statut_label}
                    disabled={disabled}
                    onValueChange={(v) => {
                      updateLabel(lbl.local_id, { statut_label: v });
                      if (onActivity) void onActivity();
                    }}
                  >
                    <SelectTrigger className="h-9 text-xs bg-background">
                      <SelectValue placeholder={isAr ? "مقترحة أم حاصلة؟" : "Proposée/Obtenue"} />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="proposee">
                        {isAr ? "مقترحة للحصول على العلامة" : "Proposée"}
                      </SelectItem>
                      <SelectItem value="obtenue">
                        {isAr ? "حاصلة على العلامة" : "Obtenue"}
                      </SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div className="md:col-span-6 space-y-1.5">
                  <Label className="text-xs">
                    {isAr ? "سبب عدم الحصول على العلامة (إن وجد)" : "Motif de refus"}
                  </Label>
                  <SafeTextarea
                    value={lbl.motif_refus || ""}
                    onValueChange={(val) => {
                      updateLabel(lbl.local_id, { motif_refus: val });
                      if (onActivity) void onActivity();
                    }}
                    disabled={disabled}
                    placeholder={isAr ? "السبب..." : "Motif..."}
                  />
                </div>
                <div className="md:col-span-6 space-y-1.5">
                  <Label className="text-xs">{isAr ? "ملاحظات" : "Observations"}</Label>
                  <SafeTextarea
                    value={lbl.observations || ""}
                    onValueChange={(val) => {
                      updateLabel(lbl.local_id, { observations: val });
                      if (onActivity) void onActivity();
                    }}
                    disabled={disabled}
                    placeholder={isAr ? "ملاحظات إضافية..." : "Observations..."}
            ...
            {labels.length === 0 && (
              <div className="text-center py-4 text-xs text-muted-foreground border border-dashed rounded-lg">
                {isAr
                  ? "لا توجد مؤسسات مسجلة للحصول على علامة الجودة"
                  : "Aucun établissement enregistré pour le label"}
              </div>
            )}
          </div>
        </Card>
