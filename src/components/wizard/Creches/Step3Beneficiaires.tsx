...
export const Step3Beneficiaires = memo(
  ({ disabled, onActivity, rapportId }: StepComponentProps) => {
    ...
    const {
      ...
      add: addStat,
      update: updateStat,
    } = useCrStatistiquesEnfants(rapportId || null);
    const {
      items: activites,
      add: addActivite,
      update: updateActivite,
      remove: removeActivite,
    } = useCrActivitesEnfants(rapportId || null);
    const {
      items: formations,
      add: addFormation,
      update: updateFormation,
      remove: removeFormation,
    } = useCrFormationsCadres(rapportId || null);
        ...
        <Card className="p-5 sm:p-6 bg-card border-border shadow-sm border-t-4 border-t-primary">
          <div className="flex items-center gap-3 border-b border-border pb-4 mb-5">
            ...
            <div>
              <h2 className="text-lg font-bold">
                {isAr
                  ? "1. عدد الأطفال المستفيدين بمؤسسات الطفولة الصغرى"
                  : "1. Enfants bénéficiaires dans les établissements"}
              </h2>
              <p className="text-sm text-muted-foreground">
                {isAr ? "التوزيع حسب الجنس والمجال" : "Répartition par genre et milieu"}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4 border border-border p-4 rounded-lg bg-muted/5 items-start">
            <div className="md:col-span-3">
              <NumericField
                label={isAr ? "ذكور" : "Garçons"}
                value={statsEntry?.garcons || 0}
                onChange={(v) => handleStatsChange("garcons", v)}
                disabled={disabled}
              />
            </div>
            <div className="md:col-span-3">
              <NumericField
                label={isAr ? "إناث" : "Filles"}
                value={statsEntry?.filles || 0}
                onChange={(v) => handleStatsChange("filles", v)}
                disabled={disabled}
              />
            </div>
            <div className="md:col-span-3">
              <NumericField
                ...
                value={statsEntry?.urbain || 0}
                onChange={(v) => handleStatsChange("urbain", v)}
                disabled={disabled}
              />
            </div>
            <div className="md:col-span-3">
              <NumericField
                ...
                value={statsEntry?.rural || 0}
                onChange={(v) => handleStatsChange("rural", v)}
                disabled={disabled}
              />
            </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-4 border-b border-border pb-4 mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold">
                  {isAr
                    ? "2. عدد الأطفال المستفيدين من الأنشطة الإشعاعية"
                    : "2. Activités de rayonnement"}
                </h2>
              </div>
            </div>
            <Button
          ...
          <div className="space-y-3">
                ...
                <div className="md:col-span-11 grid grid-cols-1 md:grid-cols-12 gap-4">
                  <div className="md:col-span-12 space-y-1.5">
                    <Label className="text-xs">
                      {isAr
                        ? "نوعية النشاط (مثل: المسيرة الخضراء، يوم الطفل...)"
                        : "Type d'activité"}
                    </Label>
                    <SafeInput
                      value={act.nom_activite}
                      onValueChange={(val) => {
                        updateActivite(act.local_id, { nom_activite: val });
                        if (onActivity) void onActivity();
                      }}
                      disabled={disabled}
                      placeholder={isAr ? "اسم أو نوع النشاط..." : "Nom de l'activité..."}
                  ...
                  <div className="md:col-span-3">
                    <NumericField
                      ...
                      value={act.garcons}
                      onChange={(v) => {
                        updateActivite(act.local_id, { garcons: v });
                        if (onActivity) void onActivity();
                      }}
                      disabled={disabled}
                    />
                  ...
                  <div className="md:col-span-3">
                    <NumericField
                      ...
                      value={act.filles}
                      onChange={(v) => {
                        updateActivite(act.local_id, { filles: v });
                        if (onActivity) void onActivity();
                      }}
                      disabled={disabled}
                    />
                  ...
                  <div className="md:col-span-3">
                    <NumericField
                      ...
                      value={act.urbain}
                      onChange={(v) => {
                        updateActivite(act.local_id, { urbain: v });
                        if (onActivity) void onActivity();
                      }}
                      disabled={disabled}
                    />
                  ...
                  <div className="md:col-span-3">
                    <NumericField
                      ...
                      value={act.rural}
                      onChange={(v) => {
                        updateActivite(act.local_id, { rural: v });
                        if (onActivity) void onActivity();
                      }}
                      disabled={disabled}
                    />
                  ...
                  <div className="md:col-span-12 space-y-1.5">
                    ...
                    <SafeTextarea
                      ...
                      onValueChange={(val) => {
                        updateActivite(act.local_id, { observations: val });
                        if (onActivity) void onActivity();
                      }}
                      disabled={disabled}
                      placeholder={isAr ? "ملاحظات..." : "Observations..."}
            ...
            {activites.length === 0 && (
              <div className="text-center py-4 text-xs text-muted-foreground border border-dashed rounded-lg">
                {isAr ? "لا توجد أنشطة إشعاعية مسجلة" : "Aucune activité"}
              </div>
            )}
          </div>
        </Card>
        ...
        <Card className="p-5 sm:p-6 bg-card border-border shadow-sm">
          <div className="flex flex-col sm:flex-row justify-between gap-4 border-b border-border pb-4 mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold">
                  {isAr ? "3. عدد الأطر المستفيدة من برامج التكوين" : "3. Formations des cadres"}
                </h2>
              </div>
            </div>
            <Button
          ...
          <div className="space-y-3">
              ...
              >
                <div className="md:col-span-11 grid grid-cols-1 md:grid-cols-12 gap-4">
                  <div className="md:col-span-5 space-y-1.5">
                    <Label className="text-xs">
                      {isAr ? "مجال التكوين" : "Domaine de formation"}
                    </Label>
                    <SafeInput
                      value={form.domaine_formation}
                      onValueChange={(val) => {
                        updateFormation(form.local_id, { domaine_formation: val });
                        if (onActivity) void onActivity();
                      }}
                      disabled={disabled}
                      placeholder={isAr ? "مجال التكوين..." : "Domaine..."}
                      className="h-9 text-xs bg-background"
                    />
                  </div>
                  ...
                  <div className="md:col-span-3">
                    <NumericField
                      label={isAr ? "عدد الأطر المستفيدة" : "Nombre de cadres"}
                      value={form.nombre_cadres}
                      onChange={(v) => {
                        updateFormation(form.local_id, { nombre_cadres: v });
                        if (onActivity) void onActivity();
                      }}
                      disabled={disabled}
                    />
                  ...
                  <div className="md:col-span-4 space-y-1.5">
                    <Label className="text-xs">
                      {isAr ? "مدة التكوين" : "Durée de la formation"}
                    </Label>
                    <div className="flex gap-2">
                      <div className="w-1/2">
                        <NumericField
                          label=""
                          value={form.duree_valeur}
                          onChange={(v) => {
                            updateFormation(form.local_id, { duree_valeur: v });
                            if (onActivity) void onActivity();
                          }}
                          disabled={disabled}
                        />
                      ...
                      <div className="w-1/2">
                        <Select
                          ...
                          disabled={disabled}
                          onValueChange={(v) => {
                            updateFormation(form.local_id, { duree_unite: v });
                            if (onActivity) void onActivity();
                          }}
                        >
                  ...
                  <div className="md:col-span-12 space-y-1.5">
                    ...
                    <SafeTextarea
                      ...
                      onValueChange={(val) => {
                        updateFormation(form.local_id, { observations: val });
                        if (onActivity) void onActivity();
                      }}
                      disabled={disabled}
                      placeholder={isAr ? "ملاحظات..." : "Observations..."}
            ...
            {formations.length === 0 && (
              <div className="text-center py-4 text-xs text-muted-foreground border border-dashed rounded-lg">
                {isAr ? "لا توجد برامج تكوين مسجلة" : "Aucune formation"}
              </div>
            )}
          </div>
        </Card>
