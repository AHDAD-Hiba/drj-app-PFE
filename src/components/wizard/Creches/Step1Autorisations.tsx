...
export const Step1Autorisations = memo(
  ({ disabled, onActivity, rapportId }: StepComponentProps & { rapportId?: string | null }) => {
    ...
    const {
      ...
      remove: removeDemande,
    } = useCrDemandesLicences(rapportId || null);
    const {
      items: traitements,
      add: addTraitement,
      update: updateTraitement,
    } = useCrTraitementLicences(rapportId || null);
    ...
    const {
      ...
      update: updateCreche,
      remove: removeCreche,
    } = useDirCrechesPrivees(effectiveDirectionId);
    ...
    const handleTraitementChange = useCallback(
      (field: string, value: any) => {
        if (traitementEntry) {
          ...
          addTraitement({
            local_id: crypto.randomUUID(),
            nombre_demandes_traitees: 0,
            delai_moyen_traitement_jours: 0,
            observations: "",
            [field]: value,
          } as any);
        }
        ...
        <Card className="p-5 sm:p-6 bg-card border-border shadow-sm border-t-4 border-t-primary">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border pb-4 mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold">
                  {isAr
                    ? "1. طلبات الترخيص لفتح وتسيير دور الحضانة الخاصة"
                    : "1. Demandes de Licences"}
                </h2>
                <p className="text-sm text-muted-foreground">
                  {isAr
                    ? "متابعة الطلبات الجديدة، التجديد، والتحويل"
                    : "Suivi des demandes d'ouverture, renouvellement et modification"}
                </p>
              </div>
            </div>
            ...
            <Button
              ...
              disabled={disabled || refTypesDemande.length === 0}
              className="gap-1.5"
            >
            ...
            {demandes.map((dem) => (
              ...
              >
                ...
                <div className="md:col-span-3 space-y-1.5">
                  ...
                  <Select
                    value={dem.type_demande_id}
                    disabled={disabled}
                    onValueChange={(v) => {
                      updateDemande(dem.local_id, { type_demande_id: v });
                      if (onActivity) onActivity();
                    }}
                  >
                    ...
                    <SelectContent>
                      {refTypesDemande.map((t) => (
                        <SelectItem key={t.id} value={t.id}>
                          {isAr ? t.libelle_ar : t.libelle_fr}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                  {dem.type_demande_id === typeAutreId && (
                    <SafeInput
                      placeholder={isAr ? "تحديد نوع الطلب..." : "Préciser..."}
                      value={dem.type_demande_autre || ""}
                      onValueChange={(value) => {
                        updateDemande(dem.local_id, { type_demande_autre: value });
                        if (onActivity) onActivity();
                      }}
                      disabled={disabled}
                      className="h-9 mt-1.5 text-xs bg-background border-primary/50"
                    />
                  )}
                </div>
                ...
                <div className="md:col-span-3 space-y-1.5">
                  ...
                  <Select
                    value={dem.statut_demande_id}
                    disabled={disabled}
                    onValueChange={(v) => {
                      updateDemande(dem.local_id, { statut_demande_id: v });
                      if (onActivity) onActivity();
                    }}
                  >
                    <SelectTrigger className="h-9 bg-background text-xs">
                  ...
                  {dem.statut_demande_id === statutAutreId && (
                    <SafeInput
                      placeholder={isAr ? "تحديد الوضعية..." : "Préciser..."}
                      value={dem.statut_demande_autre || ""}
                      onValueChange={(value) => {
                        updateDemande(dem.local_id, { statut_demande_autre: value });
                        if (onActivity) onActivity();
                      }}
                      disabled={disabled}
                      className="h-9 mt-1.5 text-xs bg-background border-primary/50"
                ...
                <div className="md:col-span-2">
                  <NumericField
                    label={isAr ? "عدد الطلبات" : "Nombre"}
                    value={dem.nombre_demandes}
                    onChange={(v) => {
                      updateDemande(dem.local_id, { nombre_demandes: v });
                      if (onActivity) onActivity();
                    }}
                    disabled={disabled}
                  />
                ...
                <div className="md:col-span-3 space-y-1.5">
                  ...
                  <SafeTextarea
                    value={dem.observations || ""}
                    onValueChange={(value) => {
                      updateDemande(dem.local_id, { observations: value });
                      if (onActivity) onActivity();
                    }}
                    disabled={disabled}
        ...
        <Card className="p-5 sm:p-6 bg-card border-border shadow-sm">
          <div className="flex items-center gap-3 border-b border-border pb-4 mb-5">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {isAr
                  ? "إحصائيات حول سرعة معالجة الملفات بالمديرية"
                  : "Indicateurs de performance de traitement"}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4 border border-border/60 p-4 rounded-lg bg-muted/5 items-center">
            ...
            <div className="md:col-span-5 space-y-1.5">
              <Label className="text-xs">
                {isAr ? "توضيحات وملاحظات" : "Remarques / Explications"}
              </Label>
              <SafeTextarea
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border pb-4 mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold">
                  {isAr
                    ? "3. سجل دور الحضانة الخاصة بالمديرية"
                    : "3. Répertoire des Crèches Privées"}
                </h2>
                <p className="text-sm text-muted-foreground">
                  {isAr
                    ? "لائحة المؤسسات الحاصلة على رخصة (يتم حفظها في سجل المديرية)"
                    : "Liste des établissements privés (Sauvegardé dans le répertoire provincial)"}
                </p>
              </div>
            </div>
            ...
            <Button
              type="button"
              size="sm"
              onClick={handleAddCreche}
              disabled={disabled || !effectiveDirectionId}
              className="gap-1.5"
            >
              <Plus className="h-4 w-4" />
              ...
              >
                <div className="md:col-span-3 space-y-1.5">
                  ...
                  <SafeInput
                    value={cr.nom_creche}
                    onValueChange={(value) => {
                      updateCreche(cr.local_id, { nom_creche: value });
                      if (onActivity) onActivity();
                    }}
                    disabled={disabled}
                    className="h-9 bg-background text-xs"
                ...
                <div className="md:col-span-2">
                  <NumericField
                    label={isAr ? "الطاقة الاستيعابية" : "Capacité"}
                    value={cr.capacite}
                    onChange={(v) => {
                      updateCreche(cr.local_id, { capacite: v });
                      if (onActivity) onActivity();
                    }}
                    disabled={disabled}
                  />
                ...
                <div className="md:col-span-3 space-y-1.5">
                  <Label className="text-xs">
                    {isAr ? "نوع/رقم الرخصة" : "Type / N° d'autorisation"}
                  </Label>
                  <SafeInput
                    value={cr.type_autorisation || ""}
                    onValueChange={(value) => {
                      updateCreche(cr.local_id, { type_autorisation: value });
                      if (onActivity) onActivity();
                    }}
                    disabled={disabled}
                    className="h-9 bg-background text-xs"
                ...
                <div className="md:col-span-3 space-y-1.5">
                  ...
                  <SafeInput
                    type="date"
                    value={cr.date_autorisation || ""}
                    onValueChange={(value) => {
                      updateCreche(cr.local_id, { date_autorisation: value });
                      if (onActivity) onActivity();
                    }}
                    disabled={disabled}
                    className="h-9 bg-background text-xs"
                ...
                <div className="md:col-span-12 space-y-1.5 pt-2 border-t border-border/50">
                  <Label className="text-xs">{isAr ? "ملاحظات" : "Observations"}</Label>
                  <SafeTextarea
                    value={cr.observations || ""}
                    onValueChange={(value) => {
                      updateCreche(cr.local_id, { observations: value });
                      if (onActivity) onActivity();
                    }}
                    disabled={disabled}
                    className="h-9 bg-background text-xs"
