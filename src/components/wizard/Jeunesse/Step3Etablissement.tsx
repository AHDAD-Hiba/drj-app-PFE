...
export const Step3Etablissement = memo(
  ({ rapportId, disabled, onActivity }: StepComponentProps) => {
    ...
    const effectiveDirectionId = rapport?.direction_id || profile?.direction_id;
    ...
    const filteredItems = useMemo(() => {
      return items.filter(
        (item) => !item.id || item.name?.toLowerCase().includes(searchTerm.toLowerCase()),
      );
    }, [items, searchTerm]);
            ...
            {filteredItems.map((it, idx) => (
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div
                    className={`space-y-1.5 ${it.project_status !== "ferme" ? "sm:col-span-2" : ""}`}
                  >
                    <Label className="text-xs font-semibold">
