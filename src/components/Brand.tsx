...
export const BrandMark = ({ size = "md" }: { size?: "sm" | "md" | "lg" }) => {
  ...
  return (
    <div
      className={cn(
        "relative flex items-center justify-center rounded-xl bg-white p-1 shadow-elegant ring-1 ring-border",
        dim,
      )}
    >
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex flex-col leading-tight">
        ...
        {!compact && <span className="text-[11px] text-muted-foreground">{t("app.subtitle")}</span>}
      </div>
    </div>
  );
};
