  ...
  return (
    <section className="space-y-3">
      ...
      <Card className={`p-4 sm:p-5 ${className}`.trim()}>
        <div className={gridClassName}>{metrics.map((metric, index) => {
          ...
          return (
            ...
            >
              ...
              <div className={`h-11 w-11 rounded-xl flex items-center justify-center ${metric.iconContainerClass}`}>
                <Icon className="h-5 w-5" />
              </div>
              <div className="space-y-1">
                <div className={`text-3xl font-extrabold tracking-tight text-foreground tabular-nums ${metric.valueClassName ?? ""}`}>
                  {metric.value}
                </div>
