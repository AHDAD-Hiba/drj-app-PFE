...
export const RegionMap = ({
  ...
  return (
    <div className={`relative w-full ${className}`} style={{ height }}>
      ...
      {hoveredPref && (
        <div
          className={`pointer-events-none absolute top-3 bg-card/95 backdrop-blur border border-border rounded-xl shadow-elegant p-3 text-sm min-w-[180px] animate-fade-in ${isAr ? "start-3" : "end-3"}`}
        >
          <div className="font-bold text-foreground text-start">
            {isAr ? hoveredPref.name_ar : hoveredPref.name_fr}
          </div>
          <div className="text-[10px] text-muted-foreground mb-2 font-mono text-start">
            {hoveredPref.code}
          </div>
          {hoveredSub ? (
