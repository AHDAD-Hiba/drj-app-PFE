...
export const computeTotalBeneficiaries = (s: Submission) =>
  ...
  (s.perm_sportive ?? 0) +
  (s.perm_capacity ?? 0) +
  (s.outreach_educative ?? 0) +
  (s.outreach_cultural ?? 0) +
  (s.outreach_sportive ?? 0) +
  (s.outreach_capacity ?? 0) +
  (s.camping_participants ?? 0) +
  (s.festivals_participants ?? 0) +
  (s.integration_beneficiaries ?? 0);
