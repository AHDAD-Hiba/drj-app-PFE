...
export function computeAfCompleteness(data: AfCompletenessData): number {
  const { 
    clubs = [], ofppt = [], laureates = [], agr = [], sensi = [], 
    portes = [], ecoute = [], rh = [], cadres = [], reseau = [], partenariats = [] 
  } = data;
  ...
  const stepCompletions = [
    ...
    countCompleted([
      ...
      clubs.some((c) => hasText(c.etablissement_id)) || ofppt.some((o) => hasText(o.etablissement_id)),
      clubs.some((c) => hasText(c.filiere_id)) || ofppt.some((o) => hasText(o.filiere_id)),
      clubs.some((c) => hasText(c.type_formation)) || ofppt.some((o) => hasText(o.niveau_formation)),
    ...
    countCompleted([
      ...
      laureates.some((l) => hasText(l.type_formation)) || agr.some((a) => hasText(a.etablissement_id)),
    ...
    countCompleted([
      ...
      sensi.some((s) => hasText(s.type_activite_id)) || portes.some((p) => hasText(p.type_activite_id)),
      ...
      sensi.some((s) => hasValue(s.benef_urbain) || hasValue(s.benef_rural)) || portes.some((p) => hasValue(p.nombre_beneficiaires)),
    ]),
    ...
    countCompleted([
      ...
      rh.some((r) => hasText(r.etablissement_id)) || cadres.some((c) => hasText(c.domaine_formation)),
    ...
    countCompleted([
      reseau.length > 0,
      reseau.some((r) => hasText(r.type_mise_a_jour)),
    ]),
