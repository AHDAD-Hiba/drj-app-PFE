import { computeCompleteness, countCompleted, hasPositiveNumber, hasText } from "@/lib/formSchema";
...
export function computeJeunesseCompleteness(data: JeunesseCompletenessData): number {
  ...
  const closedFacilities = facilities.filter(
    (facility: any) => facility.project_status === "ferme",
  );

  const hasFacilityEntries = facilities.some(
    (facility: any) =>
  ...
  const hasRequiredClosureDetails =
    ...
    (closedFacilities.length === 0 ||
      closedFacilities.some(
        (facility: any) => hasText(facility.other_status) || hasText(facility.closure_status),
      ));

  const hasCampingParticipants = camps.some(
    (camp: any) =>
      hasText(camp.programme_id) || hasPositiveNumber(camp.girls) || hasPositiveNumber(camp.boys),
  );
  ...
  const hasCampingFormations = formations.some(
    (formation: any) =>
  ...
  const hasFestivalEntries = festivals.some(
    (festival: any) =>
  ...
  const hasSocioEntries = socios.some(
    (item: any) =>
      hasText(item.sujet) ||
